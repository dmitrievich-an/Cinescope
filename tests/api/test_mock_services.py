...
class WorldClockResponse(BaseModel):
    ...
    # используем вместо устаревшего варианта с class Config
...
def get_worldclockapi_time() -> WorldClockResponse:
    ...
    response = requests.get("https://worldclockapi.com/api/json/utc/now")  # Запрос в реальный сервис
...
class TestTodayIsHolidayServiceAPI:
    # worldclockap
    @pytest.mark.skip(reason="Сервис worldclockapi работает нестабильно, используем заглушки")
    ...
    @pytest.mark.skip(reason="Сервис worldclockap работает нестабильно, используем заглушки")
...
class TestTodayIsHolidayServiceAPIWithMock:
    """Реализация заглушек с Mock в тестах pytest-mock"""

    def test_what_is_today_by_mock(self, mocker):
        # Создаем мок для функции get_worldclockapi_time
        mocker.patch(
    ...
    # Создаем Stub для функции get_worldclockapi_time
    def stub_get_worldclockapi_time(self):
    ...
    def test_what_is_today_by_stub(self, monkeypatch):
        # Подменяем реальную функцию get_worldclockapi_time на Stub
        monkeypatch.setattr("test_mock_services.get_worldclockapi_time", self.stub_get_worldclockapi_time)
        # или же можем просто напрямую взять значение из Stub world_clock_response = stub_get_worldclockapi_time()

        # Выполним тело предыдущего теста еще раз
        world_clock_response = get_worldclockapi_time()  # Произойдет вызов Stub, возвращающего "2025-01-01T00:00Z"
    ...
    def test_fake_worldclockap(self):  # проверка работоспособности сервиса worldclockap
        world_clock_response = TestTodayIsHolidayServiceAPIWithMock.get_fake_worldclockap_time()
        # Выводим текущую дату и время
        current_date_time = world_clock_response.currentDateTime
        # print(f"Текущая дата и время: {current_date_time=}")
