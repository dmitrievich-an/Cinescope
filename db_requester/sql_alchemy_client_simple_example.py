...
def sdl_alchemy_SQL():
    ...
    # Выполняем запрос
    with engine.connect() as connect:  # выполняем соединенеи с базой данных и автоматически закрываем его по завершени
        # выполнения
        result = connect.execute(text(query), {"user_id": user_id})
        for i in result:
            print(i)
